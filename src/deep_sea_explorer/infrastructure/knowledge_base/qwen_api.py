"""Small OpenAI-compatible client used for offline label-description generation."""
...
class QwenApiGenerator:
    ...
    def generate(self, image_path: Path, prompt: str) -> str:
        ...
        try:
            ...
            raise QwenApiError(f"Qwen API request failed: {type(error).__name__}") from error
        try:
            body = response.json()
