"""Server-only tests against the running local model API and real model files."""
...
pytestmark = pytest.mark.skipif(
    ...
    reason="requires the server GPU, model files, and a running loopback model service",
)
pytestmark = [
    pytestmark,
...
def _settings() -> Settings:
    ...
    if not source.model_service_auth_token:
        pytest.fail("MODEL_SERVICE_AUTH_TOKEN is required when RUN_LOCAL_MODEL_TESTS=1")
    return replace(
        source,
        model_backend=ModelBackend.REMOTE,
        model_service_enabled=True,
        model_service_base_url=os.getenv("LOCAL_MODEL_SERVICE_BASE_URL", "http://127.0.0.1:19000"),
...
def _media(root: Path) -> tuple[Path, Path]:
    ...
    image = root / f"local-model-{uuid.uuid4().hex}.jpg"
    video = root / f"local-model-{uuid.uuid4().hex}.mp4"
    import numpy as np
...
def test_server_model_api_runs_all_real_models() -> None:
    settings = _settings()
    ...
    image_path, video_path = _media(settings.temp_dir)
    try:
